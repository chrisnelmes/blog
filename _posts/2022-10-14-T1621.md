...
<ol>
  <li>Generate PAP Radius request from Security Testers computer to Network Policy Server containing valid username and any password</li>
  <li>Network Policy Server will request Multi-Factor Authentication from Azure AD</li>
  <li>Target of the security test will receive an MFA request using their default MFA method</li>
  <li>Azure AD will report success or failure to NPS</li>
  <li>Access-Accept message will be sent back to the Security Testers PC if MFA successful or Accept-Reject if not</li>
</ol>

# Implementation
- Install Network Policy Server on new or existing server with Active Directory Environment  
...
-Modify the connection request policy to accept users without validating   
    ![]({{ site.url }}/assets/T1261_1.png)
- Clone https://github.com/frontporch/Radius.NET and open in Visual Studio. Upon opening the project you may be prompted to upgrade the project to .net framework 4.8 accept this.  
- Modify the timeout value in RadiusClient.CS, this is the amount of time the radius client will wait for a response and is approximately the amount of time the user will get to accept/deny the prompt.   
  ```
    private const int DEFAULT_SOCKET_TIMEOUT = 60000
  ```
- In Program.cs of ClientTest comment out the following lines. This will allow the program to close once the radius request has finished
```
...
- Test the process by running the ClientTest.exe binary from windows command prompt  
  ...
  ```
- The target user should receive an MFA request. If the user accepts you should see Access-Accept in the terminal windows or if the MFA request is dismissed or times out Access-Reject

# Orchestatration
The following powershell script will gather a list of users from Azure Active Directory and then send an unsolicitied MFA request to this user recording the result within a powershell object. 

**Warning: This code is provided an example only, please test and validate in your own environment before using in production.** 
The following script will generate an MFA request for all AzureAD users. This be filtered by modifying the definition of $users with a filter or where-object clause.
...
```
The test results can be exported to CSV using
